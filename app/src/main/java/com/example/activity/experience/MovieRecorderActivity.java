...
public class MovieRecorderActivity extends Activity implements OnClickListener {
    // 程序中的两个按钮，录制和停止
        ...
        switch (source.getId()) {
            ...
            case R.id.record:
                if (!Environment.getExternalStorageState().equals(
                    ...
                    Toast.makeText(MovieRecorderActivity.this, "SD卡不存在，请插入SD卡！",
                            Toast.LENGTH_SHORT).show();
                    return;
                }
                try {
                    // 创建保存录制视频的视频文件，视频结果会保存在sd卡的根目录
                    File dir=new File(Environment.getExternalStorageDirectory().getAbsolutePath()+File.separator+"chinaso");
                    if(!dir.exists()){
                        dir.mkdir();
                    }
                    Time t=new Time();
                    ...
                    String filename="chinaso_video_"+year+month+day+hour+minute+second+".mp4";
                    videoFile = new File(dir+File.separator+filename);
